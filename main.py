...
@client.tree.command(
    name="pinga", description="Genera una imagen o GIF a partir de un patrón.")
@app_commands.describe(texto="Patrón en texto.",
                       gif="¿Visualizar animado en GIF? por defecto: False.",
                       bpm="Velocidad del GIF, por defecto: 120.")
async def pinga(interaction: discord.Interaction,
                texto: str,
                gif: bool = False,
                bpm: float = 120.0):
    try:
        ...
        try:
            ...
            image_paths = [
                f"{patterns_folder_path}/{pattern[0]}.png"
            ...
            if gif:
                ...
                total_scroll_width = total_width * 2 + sum(img.size[0]
                                                           for img in images)

                frames = []
                for offset in range(
                        0,
                    (total_scroll_width - total_width + 1) * precision_factor,
                        int(frame_duration_ms * precision_factor *
                            (bpm / 120.0) * 0.895)):
                    frame = Image.new("RGBA", (total_width, max_height),
                                      (255, 255, 255, 0))
                    x_offset = total_width * precision_factor - offset
                    for index, img in enumerate(images):
                        if x_offset + img.size[
                                0] * precision_factor > 0 and x_offset < total_width * precision_factor:
                            frame.paste(img,
                                        (int(x_offset / precision_factor), 0),
                                        img)
                        x_offset += img.size[0] * precision_factor
                        x_offset = int(x_offset - max_height *
                                       patterns[index][1] * precision_factor)
                ...
                if frames:
                    with io.BytesIO() as image_binary:
                        ...
                        await interaction.followup.send(file=discord.File(
                            fp=image_binary,
                            filename='trollobot_taiko_patterns.gif'))
                else:
                    await interaction.followup.send(
                        "No hay frames para crear el GIF.")
                    ...
                    with io.BytesIO() as image_binary:
                        ...
                        if i == 0:
                            await interaction.followup.send(file=discord.File(
                                fp=image_binary,
                                filename=
                                f'trollobot_taiko_pattern{i // chunk_size + 1}.png'
                            ))
                        else:
                            await interaction.channel.send(file=discord.File(
                                fp=image_binary,
                                filename=
                                f'trollobot_taiko_pattern{i // chunk_size + 1}.png'
                            ))
...
@client.tree.command(
    name="tt",
    description="Genera un mensaje con emojis a partir de un patrón.")
async def tt(interaction: discord.Interaction, texto: str):
    try:
        ...
        try:
            ...
            if len(result) > 1500:
                raise ValueError(
                    "El mensaje se pasa del límite de 1500 caracteres de Discord."
                )

            await interaction.response.send_message(result)

        except ValueError as ve:
            await interaction.response.send_message(f"Error. {ve}",
                                                    ephemeral=True)
